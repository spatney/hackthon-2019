    ...
    async execCommand(command) {
        switch (command.device) {
            case 'ip':
                ...
                setTimeout(async () => {
                    let pip;
                    try { pip = await publicIp.v4(); }
                    catch (e) { };

                    this.socket.emit('pip_result', {
                        public: pip,
                    });
                }, 0);
                ...
                setTimeout(async () => {
                    let iip;
                    try { iip = await internalIp.v4(); }
                    catch (e) { };

                    this.socket.emit('iip_result', {
                        internal: iip
                    });
                }, 0);
            ...
            case 'identify':
                ...
                const buf = new Camera().getImageData();
                this.socket.emit('image', { image: true, buffer: buf.toString('base64') });
                break;
